...
def doctor(connect, cursor, table_name, driver_path):
    ...
    for i in range(len(name_kor)):
        cursor.execute("insert into "+table_name+"(name_kor, belong,major,education,career,link,hospital_code) values(%s, %s,%s,%s,%s,%s,%s);",(name_kor[i],belong, major[i],education[i],career[i],link[i],hospital_code))
    connect.commit()
