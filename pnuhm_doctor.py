...
def doctor(connect, cursor, table_name, driver_path):
    ...
    for urlitem in BUSANURLLIST:
        ...
        for i in range(len(doctorlist)):
            ...
            ### 학력 수집 ###
            education=""
            educationlist=wd.find_elements_by_css_selector("#contents_area > article > div.introGrp.grpBlue > div > div:nth-child(3) > table > tbody > tr > td")
            ...
            career=""
            time.sleep(1)
            careerlist=wd.find_elements_by_css_selector("#contents_area > article > div.introGrp.grpBlue > div > div:nth-child(5) > table > tbody > tr > td")
            for j in careerlist:
                if (j.text!=""):
            ...
            ### mysql insert ###
            cursor.execute("INSERT INTO " + table_name + "(name_kor, belong, major, education, career, link, hospital_code) values (%s, %s, %s, %s, %s, %s, %s);", (name_kor, belong, major, education, career, link, hospital_code))
            connect.commit()
